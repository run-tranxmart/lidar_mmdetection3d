    ...
    def parse_ann_info(self, info: dict) -> Union[dict, None]:
        ...
        if len(instances) == 0:
            ...
            for ann_name in keys:
                ...
                if 'label' in ann_name and ann_name != 'attr_label' and ann_name != 'gt_ori_labels':
                    # temp_anns = [
                    ...
                    for item in temp_anns:
                        if item in self.label_mapping:
                            temp_anns_list.append(self.label_mapping[item])
                        else:
                            # print_log(f'Label ID {item} is not in label mapping, append -1')
                            temp_anns_list.append(-1)
                    temp_anns = temp_anns_list
                ...
                # breakpoint()
                if 'label' in ann_name and ann_name != 'gt_ori_labels' :
