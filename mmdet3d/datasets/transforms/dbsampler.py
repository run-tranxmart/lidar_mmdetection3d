    ...
    def __init__(
        ...
        if isinstance(info_path, str):
            if osp.exists(info_path):
                ...
                self.info_path = [osp.join(self.data_root, info_path)]
        elif isinstance(info_path, List):
            for db_path in info_path:
                if osp.exists(db_path):
                    self.info_path.append(db_path)
                else:
                    self.info_path.append(osp.join(self.data_root, db_path))
        ...
        db_infos = {}
        for db_path in self.info_path:
            with get_local_path(db_path, backend_args=self.backend_args) as local_path:
                # loading data from a file-like object needs file format
                single_db_infos = mmengine.load(
                    open(local_path, "rb"), file_format="pkl"
                )
                for k, v in single_db_infos.items():
                    src_db_len = 0
                    if k in db_infos:
                        src_db_len = len(db_infos[k])
                        db_infos[k] += single_db_infos[k]
                    else:
                        db_infos[k] = single_db_infos[k]
                    des_db_len = len(db_infos[k])
                    print(f"Db info Cls {k} : {src_db_len} -> {des_db_len}")
...
@TRANSFORMS.register_module()
class ShortRangeSampler(object):
    """Class for sampling data from the short range database.
    ...
    def __init__(
        ...
        for db_path in self.info_path:
            with get_local_path(db_path, backend_args=self.backend_args) as local_path:
                ...
                db_infos+=single_db_infos

        print(f"Db info number: {len(db_infos)}")

        self.db_infos = db_infos
        self.sampler_dict = BatchSampler(sampled_list=db_infos, name='short_range_clouds', shuffle=True)
    ...
    def sample_all(
        ...
        """Sampling short_range data.

        Args:
            gt_bboxes (np.ndarray): Ground truth bounding boxes.
            gt_labels (np.ndarray): Didn't used in this class. It is just for compatibility with other parts.
            img (np.ndarray, optional): Didn't used in this class. It is just for compatibility with other parts.
            ground_plane (np.ndarray, optional): Didn't used in this class. It is just for compatibility with other parts.
                Defaults to None.

        Returns:
            dict: Dict of sampled short range data.
        ...
        sampled_data = self.data_sampler(gt_bboxes)
        if sampled_data['instances']:
            sampled_gt_box = np.stack(
                    [instance['bbox_3d'] for instance in sampled_data['instances']], axis=0
                )
            gt_labels = np.array(
                [instance['bbox_label_3d'] for instance in sampled_data['instances']], dtype=np.longlong
            )
        else:
            sampled_gt_box = None
            gt_labels = None
        ret = None
        file_path = (
            os.path.join(self.data_root, sampled_data['lidar_points']["lidar_path"])
            if self.data_root
            else sampled_data['lidar_points']["lidar_path"]
        )
        results = dict(lidar_points=dict(lidar_path=file_path))
        s_points = self.points_loader(results)["points"]
        ...
        ret = {
            "gt_labels_3d": gt_labels,
            "gt_bboxes_3d": sampled_gt_box,
            "points": s_points,
        }

        return ret

    def data_sampler(self, gt_bboxes: np.ndarray) -> List[dict]:
        """Sampling data from short_range data and validate its objects.

        Args:
            gt_bboxes (np.ndarray): Ground truth boxes.
        Returns:
            dict: The sample with valid objects.
        """
        sampled = self.sampler_dict.sample(1)
        sampled = copy.deepcopy(sampled)
        sampled = sampled[0]
        num_gt = gt_bboxes.shape[0]
        ...
        if sampled['instances']:
            ...
            for instance in sampled['instances']:
                shr_boxes.append(instance['bbox_3d'])
            num_sampled = len(shr_boxes)
            sp_boxes = np.stack(shr_boxes, axis=0)
            ...
            for i in range(num_gt, num_gt + num_sampled):
                if coll_mat[i].any():
                    coll_mat[i] = False
                    coll_mat[:, i] = False
                    indices_to_remove.append(i - num_gt)
            sampled['instances'] = [instance for j, instance in enumerate(sampled['instances']) if j not in indices_to_remove]
            return sampled
        else:
            return sampled