    ...
    def _load_points(self, pts_filename: str) -> np.ndarray:
        ...
        try:
            if pts_filename.endswith('.pcd'):
                pcd_cloud = pypcd.point_cloud_from_path(pts_filename)
        ...
        # print("Points flags (loading): ", points.flags)
        return points

    def load_points(self, pts_filename: str) -> np.ndarray:
        """Method to load points data from file without transform.
        Args:
            pts_filename (str): point cloud path
        Returns:
            np.ndarray: An array containing point clouds data.
        """
        ...
        if points.shape[1] >= 4 and np.amax(points[:, 3]) <= 1.0:
            points[:, 3] *= 255.0
        return points
