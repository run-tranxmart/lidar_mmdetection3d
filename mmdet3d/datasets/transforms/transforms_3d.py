...
class ObjectSample(BaseTransform):
    ...
    def __init__(
        ...
        batch_indicator: Union[str, None] = None
    ) -> None:
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if self.batch_indicator and self.batch_indicator not in input_dict["lidar_path"]:
            return input_dict
        ...
        if sampled_dict is not None:
            ...
            if sampled_gt_labels is not None:
...
@TRANSFORMS.register_module()
class RemovePointsByCrowd(BaseTransform):
    """ """

    def __init__(self):
        self.rm_types = ["crowd", "Crowd"]  # ['crowd', 'Crowd']
    ...
    def transform(self, input_dict: dict) -> dict:
        """Transform function to remove crowd objects

        Args:
            input_dict (dict): Result dict from loading pipeline.

        Returns:
            dict: Results after crowd removal,
            'points', 'gt_bboxes_3d', 'gt_labels_3d' keys are updated
            in the result dict.
        """
        points = input_dict["points"]
        ...
        if "crowd" in input_dict["ann_info"].keys():
            for i, crowd_flag in enumerate(input_dict["ann_info"]["crowd"]):
                if crowd_flag == 1:
                    crowd_idxs.append(i)
                else:
                    non_crowd_idxs.append(i)
        # else:
        #     for i, flag in enumerate(input_dict["ann_info"]["truncated"]):
        #         crowd_idxs.append(i)

        if len(crowd_idxs) == 0:
            return input_dict

        bboxes = gt_bboxes_3d[crowd_idxs]
        # point_num_before = points.numpy().shape[0]
        points = self.remove_points_in_boxes(points, bboxes.tensor.numpy())
        # point_num_after = points.numpy().shape[0]
        # print("Remove crowd points: {} -> {}".format(point_num_before, point_num_after))

        input_dict["points"] = points
        # print(input_dict["ann_info"].keys())
        for key_name in [
            "gt_bboxes_3d",
            "gt_labels_3d",
            "score",
            # maybe append other keys
        ]:
            if len(non_crowd_idxs) != 0:
                input_dict["ann_info"][key_name] = input_dict["ann_info"][key_name][
                    np.array(non_crowd_idxs)
                ]
            else:
                input_dict["ann_info"][key_name] = np.array([])

        for key_name in ["gt_bboxes_3d", "gt_labels_3d"]:
            if len(non_crowd_idxs) != 0:
                input_dict[key_name] = input_dict[key_name][np.array(non_crowd_idxs)]
            else:
                input_dict[key_name] = np.array([])
        return input_dict
...
class AddGaussianNoise(BaseTransform):
    ...
    Args:
            ...
            app_ratio (float): The ratio of data to apply the transform.
    """
    def __init__(
        self,
        mean: float = 0.0,
        ...
        if app_ratio is not None:
            assert (
                isinstance(app_ratio, (int, float))
                and 0 <= app_ratio <= 1
            )
    ...
    def transform(self, input_dict: dict) -> dict:
        """
        Args:
            results (dict): The input data, containing 'points'.
        
        Returns:
            dict: The modified results with noise-added points.
        """
        points = input_dict['points']
        point_tensor = points.tensor

        # Add Gaussian noise to x, y, z coordinates
        if np.random.rand() < self.app_ratio:
            noise = np.random.normal(self.mean, self.std, point_tensor[:, :3].shape)
            point_tensor[:, :3] += noise
            input_dict['points'].tensor = point_tensor
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f"ratio={self.app_ratio})",
        return repr_str

@TRANSFORMS.register_module()
class TargetedDropoutPoints(BaseTransform):
    """
        This transform selectively drops points from 3D bounding boxes of specified
        object classes. It allows you to control the percentage of points to remove
        from each object (drop_ratio), which classes to apply the augmentation to
        (target_classes), and how often to apply it to qualifying objects (app_ratio).
        This transform is particularly useful for simulating occlusion or sensor noise
        that affects specific object categories differently, helping improve model
        robustness to varying point cloud densities for targeted object classes.
        Args:
            drop_ratio (float): Ratio of points to drop for each object.
            target_classes (list): List of classes to apply the augmentation to.
            app_ratio (float): Ratio of applying the transform to each class.
    """
    def __init__(
        self,
        ...
        assert 0 <= app_ratio <= 1, "app_ratio must be between 0 and 1"
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        points_np = points.coord.numpy()
        bboxes_np = gt_bboxes.numpy()

        masks = box_np_ops.points_in_rbbox(points_np, bboxes_np)

        for box_idx in range(masks.shape[1]):

            current_class = gt_labels[box_idx].item()
            if self.target_classes is not None and current_class not in self.target_classes:
                continue

            if np.random.random() > self.app_ratio:
                continue

            box_mask = masks[:, box_idx]
            points_in_box = np.where(box_mask)[0] 

            if len(points_in_box) > 0:
                num_points_to_drop = int(len(points_in_box) * self.drop_ratio)
        ...
        if all_drop_indices:
            all_drop_indices = np.concatenate(all_drop_indices) if all_drop_indices else np.array([])
            points = np.delete(points.numpy(), all_drop_indices, axis=0)
            input_dict['points'].tensor = torch.from_numpy(points)
        return input_dict
    ...
    def __repr__(self) -> str:
        ...
        repr_str = self.__class__.__name__
        repr_str += f"(drop_ratio={self.drop_ratio}",
        repr_str += f"target_classes={self.target_classes}",
        repr_str += f"ratio={self.app_ratio})",
        return repr_str

@TRANSFORMS.register_module()
class IntensityManipulation(BaseTransform):
    """Custom augmentation for manipulating point intensities.

    Args:
        scale_range (tuple[float, float]): The range of scaling factors to
                                           apply to the intensity values.
        app_ratio (float): Ratio of applying the transform
    """
    def __init__(self, scale_range: Union[list, tuple] = (0.8, 1.2), app_ratio: float = 0.2):
        assert isinstance(scale_range, (list, tuple)) and len(scale_range) == 2, \
            'scale_range must be a tuple of two floats.'
        assert 0 <= app_ratio <= 1, "app_ratio must be between 0 and 1"
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        points = input_dict['points']
        intensity = points.tensor[:, -1]

        scale_factor = np.random.uniform(self.scale_range[0], self.scale_range[1])
        intensity *= scale_factor

        points.tensor[:, -1] = intensity
        input_dict['points'] = points
        return input_dict
...
@TRANSFORMS.register_module()
class AddRainNoise(BaseTransform):
    """
        Add rain-like noise to the point cloud.
        Args:
            intensity (float): Fraction of additional points to simulate rain.
            app_ratio (float): Ratio of applying the transform.
    """
    def __init__(self, intensity:float =0.01, app_ratio:float = 0.2):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        point_cloud = input_dict['points'].tensor.numpy()
        num_points = point_cloud.shape[0]
        num_rain_points = int(num_points * self.intensity)

        lidar_min = np.min(point_cloud, axis=0)  # [min_x, min_y, min_z]
        lidar_max = np.max(point_cloud, axis=0)  # [max_x, max_y, max_z]

        rain_points = np.random.uniform(
            low=[lidar_min[0], lidar_min[1], lidar_min[2]],
            high=[lidar_max[0], lidar_max[1], lidar_max[2]],
            size=(num_rain_points, 3)
        )
        
        rain_points = np.hstack([rain_points, np.zeros((num_rain_points, 1))])  # Add intensity channel
        augmented_point_cloud = np.vstack((point_cloud, rain_points))
        
        input_dict['points'].tensor = torch.from_numpy(augmented_point_cloud)
        return input_dict

    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f"(intensity={self.intensity}",
        repr_str += f"ratio={self.app_ratio})",
        return repr_str

@TRANSFORMS.register_module()
class ApplyFog(BaseTransform):
    """
    Simulate fog by attenuating points based on distance with gradual removal.
    Args:
        distance_removal_rates (list of tuple):
            List of tuples specifying distance ranges and removal percentages.
            Each tuple is in the form (min_distance, max_distance, removal_rate).
        app_ratio (float): Ratio of applying the transform.
    """
    def __init__(self, distance_removal_rates=None, app_ratio=0.2):
        assert 0 <= app_ratio <= 1, "app_ratio must be between 0 and 1"

        if distance_removal_rates is None:
            # Default removal rates
            distance_removal_rates = [
                (0, 20, 0.1),  # 10% removal for distances between 0-20 meters
                (20, 50, 0.3), # 30% removal for distances between 20-50 meters
                (50, 100, 0.7),  # 70% removal for distances between 50-100 meters
                (100, np.inf, 0.9)  # 90% removal for distances beyond 100 meters
            ]

        self.distance_removal_rates = distance_removal_rates
        self.app_ratio = app_ratio
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        mask = np.ones(len(point_cloud), dtype=bool)  # Start with all points included

        for min_dist, max_dist, removal_rate in self.distance_removal_rates:
            ...
            num_to_remove = int(len(range_indices) * removal_rate)
            remove_indices = np.random.choice(range_indices, size=num_to_remove, replace=False)

            # Update the mask to exclude these points
            mask[remove_indices] = False

        # Apply the mask to the point cloud
        attenuated_cloud = point_cloud[mask]

        input_dict['points'].tensor = torch.from_numpy(attenuated_cloud)
        return input_dict
    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f"(distance_removal_rates={self.distance_removal_rates}",
        repr_str += f"ratio={self.app_ratio})",
        return repr_str


@TRANSFORMS.register_module()
class JitterPointsInBoundingBoxes(BaseTransform):
    """
    Add random jitter to points inside bounding boxes.
    
    Args:
        jitter_std (float): Standard deviation of the jitter noise.
        target_classes (list): List of classes to apply the jitter.
        app_ratio (float): Ratio of applying the transform to each class.
    """
    def __init__(self, jitter_std: float = 0.05, target_classes: list = None, app_ratio: float = 0.2):
        assert 0 <= jitter_std, "jitter_std must be non-negative"
        assert 0 <= app_ratio <= 1, "app_ratio must be between 0 and 1"
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        for box_idx in range(masks.shape[1]):
            ...
            # Apply jitter based on app_ratio
            if np.random.random() <= self.app_ratio:
                box_mask = masks[:, box_idx]
                jitter_indices = np.where(box_mask)[0]
                jitter = np.random.normal(0, self.jitter_std, size=(len(jitter_indices), 3))
                points_np[jitter_indices, :3] += jitter

        input_dict['points'].tensor = torch.from_numpy(points_np)
        return input_dict
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f"(jitter_std={self.jitter_std}",
        repr_str += f"target_classes={self.target_classes})",
        repr_str += f"app_ratio={self.app_ratio})",
        return repr_str

@TRANSFORMS.register_module()
class ModifyBackgroundNoise(BaseTransform):
    """
    Modify background noise by removing or adding points outside bounding boxes.

    Args:
        background_noise_removal_prob (float): Probability of removing a background point.
        background_noise_addition_prob (float): Fraction of points added as synthetic noise.
    """
    def __init__(self, background_noise_removal_prob: float = 0.1,
    background_noise_addition_prob: float = 0.01,
    app_ratio: float = 0.2
    ):
        assert 0 <= background_noise_removal_prob <= 1, "background_noise_removal_prob must be between 0 and 1"
        assert 0 <= background_noise_addition_prob <= 1, "background_noise_addition_prob must be between 0 and 1"
        assert 0 <= app_ratio <= 1, "app_ratio must be between 0 and 1"

        self.background_noise_removal_prob = background_noise_removal_prob
        self.background_noise_addition_prob = background_noise_addition_prob
        self.app_ratio = app_ratio
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        masks = box_np_ops.points_in_rbbox(points_np, bboxes_np)
        background_mask = ~np.any(masks, axis=1)
        background_points = np.where(background_mask)[0] 
        all_drop_indices = []

        if len(background_points) > 0:
            num_points_to_drop = int(len(background_points) * self.background_noise_removal_prob)
            if num_points_to_drop > 0:
                drop_indices = np.random.choice(
                    background_points,
                    num_points_to_drop,
            ...
            if all_drop_indices:
                all_drop_indices = np.concatenate(all_drop_indices) if all_drop_indices else np.array([])
                points_np = np.delete(points_np, all_drop_indices, axis=0)

        # Add synthetic background points
        if self.background_noise_addition_prob > 0:
            num_background_points = int(points_np.shape[0] * self.background_noise_addition_prob)
            synthetic_points = np.random.uniform(
                low=points_np[:,:3].min(axis=0), high=points_np[:,:3].max(axis=0), size=(num_background_points, 3)
            )
            # synthetic_intensity = np.zeros((num_background_points, 1))  # Assuming no intensity for synthetic points
            synthetic_intensity = np.random.uniform(low=0, high=points_np[:,-1].mean(), size=(num_background_points, 1))
            synthetic_points = np.hstack([synthetic_points, synthetic_intensity])
            synthetic_masks = box_np_ops.points_in_rbbox(synthetic_points[:, :3], bboxes_np)
            outside_bbox_mask = ~np.any(synthetic_masks, axis=1)
            synthetic_points = synthetic_points[outside_bbox_mask]
            points_np = np.vstack((points_np, synthetic_points))
    ...
    def __repr__(self) -> str:
        ...
        """Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f"(background_noise_removal_prob={self.background_noise_removal_prob}",
        repr_str += f"background_noise_addition_prob={self.background_noise_addition_prob})",
        repr_str += f"app_ratio={self.app_ratio})",
        return repr_str


@TRANSFORMS.register_module()
class RemoveEgoCloud(BaseTransform):
    """
    Removes the points within the given bounding box. Useful for removing points belonging to the ego vehicle.

    Args:
        bbox (Tuple[float, float, float, float]): The coordinates of the bounding box in the form (x_min, x_max, y_min, y_max).
    """
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        input_dict['points'].tensor = torch.from_numpy(points[mask])
        return input_dict
