...
class Det3DVisualizationHook(Hook):
    ...
    def after_test_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        for data_sample in outputs:
            ...
            if self.test_out_dir is not None:
                save_path = osp.basename(lidar_path).split('.')[0] + '.png'
                save_path = osp.join(self.test_out_dir, save_path)
            if isinstance(self._visualizer, Det3DLocalVisualizer):
                self._visualizer.add_datasample(
                    'test sample',
                    data_input,
                    ...
                    show_pcd_rgb=self.show_pcd_rgb)
            elif isinstance(self._visualizer, BEVLocalVisualizer):
                self._visualizer.add_datasample(
                    name='test sample',
                ...
                raise TypeError(
                    f'Unsupported visualization type: {self.visualizer}')
