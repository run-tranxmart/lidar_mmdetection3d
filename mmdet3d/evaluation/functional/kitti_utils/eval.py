...
def compute_statistics_jit(
    ...
    gt_overlaps = np.zeros((gt_size,))
    det_overlaps = np.zeros((det_size,))
    ...
    for i in range(gt_size):
        ...
        if (valid_detection == NO_DETECTION) and ignored_gt[i] == 0:
            ...
            # Record the overlap for gt and det
            gt_overlaps[i] = overlaps[det_idx, i]
            det_overlaps[det_idx] = overlaps[det_idx, i]
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], gt_overlaps, det_overlaps
...
def save_precision_recall(
    class_name, difficulty, precision, recall, thresholds, metric, save_dir="plots"
):
    """
    ...
    plt.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Max P*R Thresh: {max_f1_threshold:.2f}",
    )
    plt.scatter(
        [max_f1_threshold],
        [max_precision],
        color="red",
        label=f"P: {max_precision:.2f}",
        zorder=5,
    )
    plt.scatter(
        [max_f1_threshold],
    ...
    plt.text(
        max_f1_threshold,
        max_precision,
        f"P={max_precision:.2f}",
        color="red",
        fontsize=10,
    )
    plt.text(
        max_f1_threshold, max_recall, f"R={max_recall:.2f}", color="blue", fontsize=10
    )

    # Labels and Legend
    plt.title(
        f"Precision-Recall vs Threshold for {class_label} ({metric_name})", fontsize=14
    )
    ...
    # Save the plot
    plot_filename = (
        f"{save_dir}/{metric_name}_{class_label}_{difficulty}_precision_recall_plot.png"
    )
...
def eval_class(
    ...
    metric_dict = {0: "bbox-2D", 1: "BEV", 2: "3D"}
    N_SAMPLE_PTS = 41
        ...
        for idx_l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    tp, fp, fn, similarity, thresholds, gt_overlaps, det_overlaps = rets
                    # if only one frame, print the overlap information
                    if len(gt_annos) == 1:
                        if metric == 1 or metric == 2:
                            det_scores = dt_datas_list[i][:, -1]
                            print(
                                "Class: {} , Metric(Overlap): {}({}) , TP: {} , FP : {} , FN : {} ".format(
                            ...
                            print(
                                "--- Overlaps for GT : ",
                                gt_overlaps[ignored_gts[i] == 0],
                            )
                            print(
                                "--- Overlaps for Det: ",
                                det_overlaps[ignored_dets[i] == 0],
                            )
                            print(
                                "--- Scores for Det: ",
                                det_scores[ignored_dets[i] == 0],
                            )
                ...
                if total_num_valid_gt > 0 and len(thresholdss) > 0:
                    ...
                    print(
                        f"Skipping class {current_class} at difficulty {difficulty}: No valid thresholds."
                    )
                    continue
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if mAP40_bbox is not None:
                result += "bbox AP40:{:.2f}, {:.2f}, {:.2f}\n".format(
                    *mAP40_bbox[j, :, i]
                )
            if mAP40_bev is not None:
                result += "bev  AP40:{:.2f}, {:.2f}, {:.2f}\n".format(
                    *mAP40_bev[j, :, i]
                )
            if mAP40_3d is not None:
                result += "3d   AP40:{:.2f}, {:.2f}, {:.2f}\n".format(
                    *mAP40_3d[j, :, i]
                )
            if compute_aos:
...
def kitti_eval_coco_style(gt_annos, dt_annos, current_classes):
    ...
    """
    class_to_name = {0: "Car", 1: "Pedestrian", 2: "Cyclist", 3: "Truck"}
    class_to_range = {
        0: [0.5, 0.95, 10],
        1: [0.25, 0.7, 10],
        2: [0.25, 0.7, 10],
...
def custom_eval(
    gt_annos, dt_annos, current_classes, eval_types=["bbox", "bev", "3d", "aos"]
):
    """Custom evaluation.
    ...
    class_to_name = {0: "Car", 1: "Pedestrian", 2: "Cyclist", 3: "Truck", 4: "Misc"}
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            for idx in range(1):
                ...
                if mAP40_3d is not None:
                    ret_dict[f"{prefix}_3D_AP"] = mAP40_3d[j, idx, i]
                if mAP40_bev is not None:
                    ret_dict[f"{prefix}_BEV_AP"] = mAP40_bev[j, idx, i]
                if mAP40_bbox is not None:
                    ret_dict[f"{prefix}_2D_AP"] = mAP40_bbox[j, idx, i]

    # calculate mAP40 over all classes if there are multiple classes
    if len(current_classes) > 1:
        result += "\nOverall AP@{}:\n".format(*difficulty)
        if mAP40_bev is not None:
            mAP40_bev_overall = mAP40_bev.mean(axis=0)
            result += "bev  AP: {:.2f}\n".format(*mAP40_bev_overall[:, 0])
        if mAP40_3d is not None:
            mAP40_3d_overall = mAP40_3d.mean(axis=0)
            result += "3d   AP: {:.2f}\n".format(*mAP40_3d_overall[:, 0])
        if compute_aos:
            mAP40_aos_overall = mAP40_aos.mean(axis=0)
            result += "aos  AP: {:.2f}\n".format(*mAP40_aos_overall[:, 0])

        # prepare results for logger
        for idx in range(1):
            postfix = f"{difficulty[idx]}"
            if mAP40_3d is not None:
                ret_dict[f"Custom/Overall_3D_AP_{postfix}"] = mAP40_3d_overall[idx, 0]

            if mAP40_bev is not None:
                ret_dict[f"Custom/Overall_BEV_AP_{postfix}"] = mAP40_bev_overall[idx, 0]

    # prepare mean value without misc
    mAP40_bev_without_misc = mAP40_bev[:4].mean(axis=0)
    mAP40_3d_without_misc = mAP40_3d[:4].mean(axis=0)
    # print overall map without misc
    if len(current_classes) > 1:
        result += "\nOverall_without_Misc  AP@{}:\n".format(*difficulty)
        if mAP40_bev_without_misc is not None:
            result += "bev  AP: {:.2f}\n".format(*mAP40_bev_without_misc[:, 0])
        if mAP40_3d_without_misc is not None:
            result += "3d   AP: {:.2f}\n".format(*mAP40_3d_without_misc[:, 0])
        if compute_aos:
            mAP40_aos_filtered = mAP40_aos[:4].mean(axis=0)
            result += "aos  AP: {:.2f}\n".format(*mAP40_aos_filtered[:, 0])
        # prepare results for logger
        for idx in range(1):
            postfix = f"{difficulty[idx]}"
            if mAP40_bev_without_misc is not None:
                ret_dict[f"Custom/Overall_BEV AP_{postfix}"] = mAP40_bev_without_misc[
                    idx, 0
                ]
            if mAP40_3d_without_misc is not None:
                ret_dict[f"Custom/Overall_3d AP_{postfix}"] = mAP40_3d_without_misc[
                    idx, 0
                ]
