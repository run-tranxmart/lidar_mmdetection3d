...
from mmdet3d.evaluation.functional.kitti_utils.eval_fp import custom_eval_fp
...
class Custom3DMetric_fp(BaseMetric):
    ...
    def __init__(
        ...
        preload_annos: bool = False,
        filter_annos_by_range: bool = True,
        save_dir: str = 'fp_miss_labeling_frames',      
        score_threshold: float = 0.8,
    ) -> None:
        self.save_dir = save_dir
        self.score_threshold = score_threshold
        self.default_prefix = "custom 3d metric"
        super(Custom3DMetric_fp, self).__init__(
        ...
        # load annotations
        self.data_infos = []
        self.filter_annos_by_range = filter_annos_by_range
        if preload_annos:
            if isinstance(self.ann_file, str):
                ...
                for file in self.ann_file:
                    infos_list.append(load(file, backend_args=self.backend_args))
                pkl_infos = infos_list[0].copy()
                pkl_infos['data_list'] = [item for d in infos_list for item in d['data_list']]

            self.data_infos = self.convert_annos(pkl_infos)

    def convert_annos(self, data_infos: dict) -> List[dict]:
        """Convert loading annotations.
            ...
            for i, annos in enumerate(data_annos):
                if len(annos["instances"]) == 0:
                    ...
                    for instance in annos["instances"]:
                        ...
                        #This is the previous method where we used alpha directly, I commented it.
                        #kitti_annos["alpha"].append(instance["alpha"])
                        box_lidar = instance["bbox_3d"]  # This is assumed to be in LiDAR coordinates [x, y, z, l, w, h, rotation_y]
                ...
                if self.filter_annos_by_range:
                    locations = kitti_annos["location"]
                    range_indices = np.where(
                        (locations[:, 0] >= self.pcd_limit_range[0])
                        & (locations[:, 0] <= self.pcd_limit_range[3])
                        & (locations[:, 1] >= self.pcd_limit_range[1])
                        & (locations[:, 1] <= self.pcd_limit_range[4])
                        & (locations[:, 2] >= self.pcd_limit_range[2])
                        & (locations[:, 2] <= self.pcd_limit_range[5])
                    )
                    kitti_annos["name"] = kitti_annos["name"][range_indices]
                    kitti_annos["truncated"] = kitti_annos["truncated"][range_indices]
                    kitti_annos["occluded"] = kitti_annos["occluded"][range_indices]
                    kitti_annos["alpha"] = kitti_annos["alpha"][range_indices]
                    kitti_annos["bbox"] = kitti_annos["bbox"][range_indices]
                    kitti_annos["location"] = kitti_annos["location"][range_indices]
                    kitti_annos["dimensions"] = kitti_annos["dimensions"][range_indices]
                    kitti_annos["rotation_y"] = kitti_annos["rotation_y"][range_indices]
                    kitti_annos["score"] = kitti_annos["score"][range_indices]
                    # print(f"Frame {len(locations)} -> {len(range_indices[0])}")
                # l, h, w -> l, w, h
                # kitti_annos['dimensions'][:, [0, 1, 2]] = kitti_annos['dimensions'][:, [0, 2, 1]]
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ap_dict = self.custom_3d_evaluate(
                result_dict,
                gt_annos,
                metric=metric,
                logger=logger,
            ...
            if 'ret_dict_custom' in ap_dict:
                metric_dict['ret_dict_custom'] = ap_dict['ret_dict_custom']
    ...
    def custom_3d_evaluate(
        ...
        """Evaluation in custome 3d protocol.
        ...
        """
        ap_dict = dict()
        ret_dict_custom = {'ret_bbox': None, 'ret_bev': {}, 'ret_3d': {}}
        for name in results_dict:
            ...
            if name == "pred_instances" or metric == "img_bbox":
                ...
                #eval_types = ["bev", "3d"]
            ap_result_str, ap_dict_ = custom_eval_fp(
    ...
    def format_results(
        ...
        for name in results[0]:
            ...
            if (
                ...
                result_list_ = self.bbox2result_custom(
                    net_outputs,
                    sample_idx_list,
                    classes,
                    pklfile_prefix_,
                ...
                warnings.warn("NOT support pred instances 2d")
    ...
    def bbox2result_custom(
        ...
        for idx, pred_dicts in enumerate(mmengine.track_iter_progress(net_outputs)):
            ...
            if pklfile_prefix is not None:
                dump_dir = osp.dirname(pklfile_prefix)
                result_dict = {
                    'bboxes_3d': box_dict['box3d_lidar'].tolist(),
                    'scores_3d': box_dict['scores'].tolist(),
                    'labels_3d': box_dict['label_preds'].tolist(),
                    # 'dir_3d': dir_preds,
                }
                frame_name = f"{info['lidar_points']['lidar_path']}.json"
                dump_pth = osp.join(dump_dir, frame_name)
                mmengine.dump(result_dict, dump_pth)
            ...
            if len(box_dict["bbox"]) > 0:
                ...
                for box_camera, box_lidar, bbox, score, label in zip(
                    box_preds_camera, box_preds_lidar, box_2d_preds, scores, label_preds
                ):
                    ...
                    if pred_box_type_3d == CameraInstance3DBoxes:
                        # box_camera format: [x, y, z, l, w, h, rot_y]
                        # alpha = rotation_y - arctan2(x, z)
                        anno["alpha"].append(-np.arctan2(box_camera[0], box_camera[2]) + box_camera[6])
                    elif pred_box_type_3d == LiDARInstance3DBoxes:
                        # Attempting a similar formula directly in LiDAR coordinates
            ...
            if "lidar_points" in info:
                anno["lidar_path"] = info["lidar_points"]["lidar_path"]
            else:
                warnings.warn("Cannot get lidar path from data infos")
    ...
    def convert_valid_bboxes(self, box_dict: dict, info: dict) -> dict:
        ...
        if isinstance(box_preds, LiDARInstance3DBoxes):
            ...
            warnings.warn("The camera 3D instances are not supported!")
