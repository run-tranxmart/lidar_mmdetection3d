...
def box3d_multiclass_nms(mlvl_bboxes,
    ...
    for i in range(num_classes):
        ...
        output = nms_func(_bboxes_for_nms, _scores, cfg.nms_thr)
        if isinstance(output, tuple):
            selected = output[1]  # Assuming the second element is the indices
        else:
            selected = output
        
        selected = selected.long()  # Convert 'selected' to torch.long
