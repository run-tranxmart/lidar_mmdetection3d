...
class BiFPN(nn.Module):
    def __init__(self,
        ...
        for i in range(3) : 
            ...
            deblock = nn.Sequential(upsample_layer,
                                    build_norm_layer(dict(type='BN', eps=1e-3, momentum=0.01), out_channels)[1],
                                    nn.ReLU(inplace=True))
            deblocks.append(deblock)
            self.dblocks = nn.ModuleList(deblocks)
    ...
    def forward(self, inputs):
        ...
        outs = [dblock(outs[i]) for i,dblock in  enumerate(self.dblocks)]
