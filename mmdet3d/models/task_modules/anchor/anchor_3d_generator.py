...
@TASK_UTILS.register_module()
class Anchor3DSingleYawRangePerClassGenerator(object):
    """3D Anchor Generator by range.
    This anchor generator generates anchors by the given range in different
    feature levels.
    Due the convention in 3D detection, different anchor sizes are related to
    ...
    def num_base_anchors(self):
        ...
        num_rot = 1  # len(self.rotations)
        num_size = torch.tensor(self.sizes).reshape(-1, 5).size(0)
        return num_rot * num_size
    ...
    def single_level_grid_anchors(self, featmap_size, scale, device='cuda'):
        ...
        # which is ~5 times faster than the numpy implementation
        if not self.size_per_range:
            return self.anchors_single_range(featmap_size, self.ranges[0], scale, self.sizes, device=device)

        mr_anchors = []
        for anchor_range, anchor_size in zip(self.ranges, self.sizes):
            mr_anchors.append(self.anchors_single_range(featmap_size, anchor_range, scale, anchor_size, device=device))
        mr_anchors = torch.cat(mr_anchors, dim=-3)
    ...
    def anchors_single_range(self, feature_size, anchor_range, scale=1, sizes=[[1.6, 3.9, 1.56]], device='cuda'):
        """Generate anchors in a single range.
        Args:
            feature_size (list[float] | tuple[float]): Feature map size. It is
                either a list of a tuple of [D, H, W](in order of z, y, and x).
        ...
        sizes = torch.tensor(sizes, device=device).reshape(-1, 5)  # * scale
