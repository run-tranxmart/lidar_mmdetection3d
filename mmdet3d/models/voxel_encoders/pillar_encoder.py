...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, features: Tensor, num_points: Tensor, coors: Tensor,
        ...
        if self._with_cluster_center:
            # Transpose to [voxel_num, 3, 32] before ReduceSum
            features_transposed = features[:, :, :3].permute(0, 2, 1)
            points_mean = features_transposed.sum(dim=2, keepdim=True) / num_points.type_as(features).view(-1, 1, 1)
            # Transpose back to [voxel_num, 32, 3]
        ...
        if self._with_voxel_center:
            if not self.legacy:
                f_center = features[:, :, :3] - torch.stack([
    ...
    def extract_input_pfns(self, features: Tensor, num_points: Tensor,
