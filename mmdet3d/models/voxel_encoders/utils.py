...
def get_paddings_indicator(actual_num: Tensor,
    ...
    range_tensor = torch.arange(max_num, device=actual_num.device).unsqueeze(0)
    paddings_indicator = range_tensor < actual_num
...
class PFNLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_conv1d: Optional[bool] = False,
                 mode: Optional[str] = 'max'):
        '''
            Origin setting is using nn.Linear to transform the point features
            from in_channels to out_channels. When the flag of use_conv1d is
            set to True, the nn.conv1d is adopted to replace the nn.linear,
            which supports the ONNX export without extra transpose operations,
        '''
        super().__init__()
        self.name = 'PFNLayer'
        ...
        if not self.use_conv1d:
            ...
            self.matmul = nn.Conv1d(in_channels, out_channels, kernel_size=1)
    ...
    def forward(self,
        ...
        if not self.use_conv1d:
            x = self.linear(inputs)
            x = self.norm(x.permute(0, 2,
                                    1).contiguous()).permute(0, 2,
                                                             1).contiguous()
            x = F.relu(x)
        else:
            d1, d2, d3 = inputs.shape
            x = torch.unsqueeze(inputs, -1).reshape(d1 * d2, d3, 1)
            x = self.matmul(x)
            x = self.norm(x)
            # x = x.reshape(-1,d2,self.units)
            x = x.reshape(d1, d2, self.units)
            x = F.relu(x)
