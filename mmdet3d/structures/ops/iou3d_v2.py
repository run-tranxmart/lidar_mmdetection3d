...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, post_max_size=None):
    """Nms function with GPU implementation using nms.

    Args:
        boxes (torch.Tensor): Input boxes with the shape of [N, 5] ([x1, y1, x2, y2, ry]).
        scores (torch.Tensor): Scores of boxes with the shape of [N].
        thresh (float): IoU threshold for NMS.
        pre_maxsize (int, optional): Max size of boxes before NMS. Default: None.
        post_max_size (int, optional): Max size of boxes after NMS. Default: None.

    Returns:
        torch.Tensor: Indexes of boxes after NMS.
    """
    ...
    # Remove rotation component if present
    if boxes.size(1) == 5:
        boxes = boxes[:, :4]
    ...
    # Apply maximum size limits before NMS
    if pre_maxsize is not None:
        boxes = boxes[:pre_maxsize]
        scores = scores[:pre_maxsize]

    # Perform NMS
    keep = nms(boxes, scores, thresh)

    # Apply maximum size limits after NMS
