...
def export_onnx(
    ...
    with torch.no_grad():
        ...
        voxel_input_copy5 = deepcopy(voxel_input)  # copy for additional processing
        voxel_input_copy6 = deepcopy(voxel_input)  # copy for additional processing
        voxel_input_copy7 = deepcopy(voxel_input)  # copy for additional processing
        ...
        output_detector = model_pointpillar(deepcopy(voxel_input_copy5))
        ...
        onnx1_path = osp.join(save_dir, "single_stage.no_legacy.onnx")

        pytorch_output = model_pointpillar(deepcopy(voxel_input_copy6))
        ...
        torch.onnx.export(
            ...
            [
                ...
                voxel_input_copy4[1],
                voxel_input_copy4[2],
            ],  # Ensure to pass them as a tuple if they are expected as such
            onnx1_path,
            ...
            output_names=[
                ...
                "bbox",
                "dir",
            ],  # Output names as per your model
            verbose=False,
            opset_version=11,
            dynamic_axes={
                "inputs": {
                    0: "voxel_num"
                },  # The first tensor has only one dynamic dimension -> num points
                "onnx::Slice_1": {
                    0: "voxel_num"
                },  # The second tensor has a dynamic first dimension -> features
                "coors": {
                    0: "voxel_num"
                },  # The third tensor has a dynamic first dimension -> coors
            },
        )
        ...
        input_feed = {
            "inputs": voxel_input_copy7[0].numpy(),
            "onnx::Slice_1": voxel_input_copy7[1].numpy(),
            "coors": voxel_input_copy7[2].numpy(),
        }
...
def parse_config():
    parser = argparse.ArgumentParser(
        description="Convert PyTorch checkpoint to ONNX for PointPillars"
    )
    parser.add_argument(
        "--model_cfg",
        type=str,
        help="Path of config",
        default="configs/users/run.yang/configs/aicc_pointpillars_heavy_5class_no_legacy_export.py",
    )
...
if __name__ == "__main__":
    """
        Example: 
            python tools/convert_onnx_no_legacy.py \
    ...
    # Load the ONNX model
    model = onnx.load(onnx1_pth)

    # Get the input names from the model
    input_names = [input.name for input in model.graph.input]
    print("ONNX model input names are:", input_names)
