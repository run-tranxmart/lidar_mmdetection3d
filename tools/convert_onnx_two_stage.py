...
class VoxelNet(SingleStage3DDetector):
    ...
    def forward(self, voxel_list):
        ...
        # _, cls_topk_ind = cls_score_max.topk(self.topk_num)
...
def export_onnx(
    ...
    with torch.no_grad():
        ...
        voxel_input = list(model_inputs["inputs"]["voxels"].values())
        voxel_input_copy0 = deepcopy(
            voxel_input
        )  # to collect pfn (voxel-encoder) input
        voxel_input_copy1 = deepcopy(
            voxel_input
        )  # to collect pfn (voxel-encoder ) output
        voxel_input_copy2 = deepcopy(
            voxel_input
        )  # to collect scatter (middle-encoder) output
        ...
        output_detector = model_pointpillar(voxel_input_copy3)
        ...
        onnx1_path = osp.join(save_dir, "pfns.onnx")
        model_pointpillar.forward = model_pointpillar.forward_pfns
        torch.onnx.export(
            model_pointpillar,
            deepcopy(input_pfns),
            onnx1_path,
            ...
            export_params=True,
            dynamic_axes={"inputs": {0: "dynamic"}, "outputs": {0: "dynamic"}},
            input_names=["inputs"],
        ...
        ort_output_part1 = ort_session_part1.run(None, {"inputs": input_pfns.numpy()})[
            0
        ]

        # Export ONNX part2 model
        onnx2_path = osp.join(save_dir, "detector.onnx")
        model_pointpillar.forward = model_pointpillar.forward_detector
        torch.onnx.export(
            model_pointpillar,
            output_scatter,
            onnx2_path,
            ...
            export_params=True,
            input_names=["inputs"],
            output_names=["topk_index", "cls_scores", "bbox", "dir"],
            verbose=True,
            opset_version=11,
        )
        onnx.save(onnx.shape_inference.infer_shapes(onnx.load(onnx2_path)), onnx2_path)
        options = onnxruntime.SessionOptions()
        options.intra_op_num_threads = 1
        options.inter_op_num_threads = 1
        ort_session_part2 = onnxruntime.InferenceSession(
            onnx2_path, sess_options=options, providers=["CPUExecutionProvider"]
        )
        ort_output_part2 = ort_session_part2.run(
            None, {"inputs": output_scatter.detach().numpy()}
        )[1]
        print(
            "ONNX Part1 Output Similarity",
            mtx_similar1(ort_output_part1, output_pfns.detach().numpy()),
        )
        print(
            "ONNX Part2 Output Similarity",
            mtx_similar1(ort_output_part2, output_detector[1].detach().numpy()),
        )

        return onnx1_path, onnx2_path
...
def save_torch_results(
    ...
    if len(feat_data["detector_output"]) == 4:
        ...
        postprogressor = Postprocess(model_config)
...
def parse_config():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path of config",
        default="configs/users/public/configs/batch0_pointpillars_v2_2_light_export.py",
    )
    ...
    parser.add_argument(
        ...
        default="demo/data/batch_0/0_325.bin",
    )
    parser.add_argument(
        "--use_config_cloud",
        action="store_true",
...
if __name__ == "__main__":
    """
        Example: 
            python tools/convert_onnx_two_stage.py --ckpt path_to_checkpoint \
    ...
    onnx1_pth, onnx2_pth = export_onnx(
    ...
    print("Export ONNX model -> {}, {}".format(onnx1_pth, onnx2_pth))
