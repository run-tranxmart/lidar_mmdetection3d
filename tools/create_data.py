...
def custom_data_prep(root_path, info_prefix, version, out_dir):
    """Prepare data related to Custom dataset.
    ...
    Args:
        ...
        out_dir (str): Output directory of the groundtruth database info.
    """
    custom_converter.create_custom_info_file(root_path, info_prefix)

    info_train_path = osp.join(out_dir, f'{info_prefix}_infos_train.pkl')
    info_val_path = osp.join(out_dir, f'{info_prefix}_infos_val.pkl')
    info_trainval_path = osp.join(out_dir, f'{info_prefix}_infos_trainval.pkl')
    # info_test_path = osp.join(out_dir, f'{info_prefix}_infos_test.pkl')
    update_pkl_infos('custom', out_dir=out_dir, pkl_path=info_train_path)
    update_pkl_infos('custom', out_dir=out_dir, pkl_path=info_val_path)
    update_pkl_infos('custom', out_dir=out_dir, pkl_path=info_trainval_path)
    # create_groundtruth_database(
    #     'KittiDataset',
    #     root_path,
    #     info_prefix,
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help='Path to the train data information pkl file.')
parser.add_argument(
    '--version',
    type=str,
    default='v1.0',
    ...
    if args.dataset == 'kitti':
        ...
        if args.only_gt_database:
            if args.train_info is None:
                info_path = [
                    'batch_7_jac_data/pkls/batch_7_jac_infos_5cls_train.update_prefix.pkl',
                    'batch_9_jac_data/pkls/train_data_infos.update_prefix.pkl'
                    ]
                for i in range(len(info_path)):
                    info_path[i] = osp.join(args.root_path, info_path[i])

            else:
                info_path = args.train_info
            create_groundtruth_database(
                'KittiDataset',
                args.root_path,
                args.extra_tag,
                info_path = info_path,
                database_save_path = osp.join(args.out_dir, f'{args.extra_tag}_gt_database'),
