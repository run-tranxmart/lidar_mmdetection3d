...
def infer_onnx_results(
    ...
    try:
        ...
        # print("ONNX output:", ort_output_part1[0])
    ...
    if dump_results_cpp:
        ...
        # Derive preprocessed input file names for .bin format
        inputs1_file = cpp_results_dir + "/" + cpp_results_name + "_inputs1.bin"
        coors_file = cpp_results_dir + "/" + cpp_results_name + "_coors.bin"

        # Save each input separately in .bin files
        input_feed["inputs1"].tofile(inputs1_file)
...
def parse_config():
    ...
    parser.add_argument(
        "--dump", action="store_true", default=False, help="Dump internal results"
    )
    parser.add_argument(
        "--pytorch_results",
        type=str,
...
def process_frame(cloud_pth, args, cloud_params, onnx_session, cfg, cloud_loader):
