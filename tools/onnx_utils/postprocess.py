...
from mmdet3d.structures.ops.iou3d.iou3d_utils import nms_gpu, nms_normal_gpu
...
class Postprocess():
    ...
    def __init__(self, cfg):
        ...
        # Extract point_cloud from the config
        self.point_cloud = cfg.model['data_preprocessor']['voxel_layer']['point_cloud_range']
        
    
        # Compute voxel_size and other dependent variables
        self.voxel_size = cfg['model']['data_preprocessor']['voxel_layer']['voxel_size']
        dx = self.point_cloud[3] - self.point_cloud[0]
        dy = self.point_cloud[4] - self.point_cloud[1]
        feat_size = [
            round(dy / self.voxel_size[1]),
            round(dx / self.voxel_size[0]),
            1
        ]
        self.generator_featmap_sizes = [
            [round(feat_size[0] / 2), round(feat_size[1] / 2)]
        ]
        
        # Extract other attributes from the config
        bbox_head_cfg = cfg['model']['bbox_head']
        
        self.in_channels = bbox_head_cfg['in_channels']
        self.num_classes = bbox_head_cfg['num_classes']
        self.feat_channels = bbox_head_cfg['feat_channels']
        self.diff_rad_by_sin = bbox_head_cfg.diff_rad_by_sin
        self.use_direction_classifier = bbox_head_cfg.use_direction_classifier
        self.assigner_per_size = bbox_head_cfg.assigner_per_size
        self.assign_per_class = bbox_head_cfg.assign_per_class
        self.dir_offset = bbox_head_cfg['dir_offset']
        self.dir_limit_offset = bbox_head_cfg['dir_limit_offset']
        self.long_branch_num = bbox_head_cfg.long_branch_num
        self.assign_per_level = True
        self.conv_bias = bbox_head_cfg.conv_bias
        self.use_sigmoid_cls = bbox_head_cfg.loss_cls.use_sigmoid
        self.nms_score_thr = cfg['model']['test_cfg']['nms_thr']
        self.score_thr = cfg['model']['test_cfg']['score_thr']
        self.nms_max_num = cfg['model']['test_cfg']['max_num']
        
        # Generator Params
        anchor_generator_cfg = bbox_head_cfg['anchor_generator']
        self.generator_ranges = (anchor_generator_cfg['ranges'], )
        self.generator_sizes = (anchor_generator_cfg['sizes'], )
        self.generator_scales = ([1], )
        self.generator_custom_values = ([], )
        self.generator_reshape_out = (anchor_generator_cfg['reshape_out'], )
        self.generator_size_per_range = True
        # DeltaXYZWLHRBBoxCoder Params
        self.box_code_size = bbox_head_cfg['bbox_coder']['code_size']
    ...
    def box3d_multiclass_nms(self,
        ...
        for i in range(0, num_classes):
            ...
            # nms_func = self.nms_bev
            nms_func = nms_gpu
            if nms_func == nms_gpu:
                _bboxes_for_nms = _bboxes_for_nms.cuda()
                _scores = _scores.cuda()
                selected = nms_func(_bboxes_for_nms, _scores, 0.5)
    ...
    def filter_results(self, src_instances: Dict):
        ...
        limit_range = bboxes_3d.tensor.new_tensor(self.point_cloud)
    ...
    def predict_by_feat(
        ...
        cfg = Config(cfg_dict={'use_rotate_nms': True, 'nms_thr': 0.5})
        bboxes = bboxes.cuda()
