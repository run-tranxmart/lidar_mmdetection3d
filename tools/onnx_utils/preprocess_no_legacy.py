...
class Preprocess:
    ...
    def __init__(
        ...
        if self.cloud_loader is None:
            self.cloud_loader = LoadPointsFromFile(
                coord_type="LIDAR", load_dim=self.load_dim, use_dim=self.use_dim
            )
    ...
    def filter_range_3d(self):
        in_range_flags = (
            (self.points[:, 0] > self.point_cloud_range[0])
    ...
    def get_paddings_indicator(
        self, actual_num: Tensor, max_num: Tensor, axis: int = 0
    ) -> Tensor:
    ...
    def dist_PCenter(self, feat_inputs: Tensor, num_points: Tensor, coors: Tensor):
        features_ls = [feat_inputs]
        # Find distance of x, y, and z from cluster center
        if self._with_cluster_center:
            features_transposed = feat_inputs[:, :, :3].permute(0, 2, 1)
            points_mean = features_transposed.sum(
                dim=2, keepdim=True
            ) / num_points.type_as(feat_inputs).view(-1, 1, 1)
            points_mean = points_mean.permute(0, 2, 1)
        ...
        if self._with_voxel_center:
            if not self.legacy:
                f_center = feat_inputs[:, :, :3] - torch.stack(
                    [
